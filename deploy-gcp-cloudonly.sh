...
echo "🚀 MythSeeker GCP Cloud-Only Deployment"
echo "======================================="
...
# Deploy directly using Cloud Build (no local Docker needed)
print_status "Building and deploying with Cloud Build (no local Docker required)..."
gcloud builds submit --config cloudbuild.yaml .

# Get the service URL
print_status "Getting service information..."
SERVICE_URL=$(gcloud run services describe $SERVICE_NAME --region=$REGION --format="value(status.url)" 2>/dev/null || echo "Service not found")
...
if [ "$SERVICE_URL" != "Service not found" ]; then
    ...
    echo "   Update: gcloud builds submit --config cloudbuild.yaml ."
    ...
    print_error "Deployment may have failed. Check Cloud Build logs:"
    echo "gcloud builds log --limit=1"
fi 